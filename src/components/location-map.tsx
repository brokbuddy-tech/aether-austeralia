...
export function LocationMap({
  ...
  const openStreetMapUrl = hasCoordinates
    ...
    : `https://www.openstreetmap.org/search?query=${encodeURIComponent(addressLabel || locationLabel || 'Australia')}`;

  if (!markerPosition) {
    return (
      <div className="flex h-96 w-full flex-col items-center justify-center rounded-3xl border border-border bg-muted/35 px-6 text-center">
        <p className="text-lg font-semibold text-foreground">Location pin unavailable</p>
        <p className="mt-2 max-w-md text-sm text-muted-foreground">
          This listing is missing a complete latitude and longitude pair, so the map pin cannot be placed yet.
        </p>
  ...
  return (
    <div className="leaflet-property-map relative h-96 w-full overflow-hidden rounded-3xl border border-border">
      <MapContainer
      ...
      <div className="pointer-events-none absolute inset-x-0 top-0 bg-gradient-to-b from-background/90 via-background/55 to-transparent px-4 py-4">
        <p className="text-sm font-semibold text-foreground">{addressLabel || locationLabel || 'Australia'}</p>
        <p className="text-xs text-muted-foreground">
          {latitude?.toFixed(5)}, {longitude?.toFixed(5)}
        </p>
      </div>
