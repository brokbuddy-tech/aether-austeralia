...
const variantClasses: Record<ReviewCarouselVariant, {
  ...
  gold: {
    ...
    avatar: 'bg-primary text-white',
    avatarText: 'text-white',
    button: 'border border-primary/30 bg-transparent text-primary hover:bg-primary hover:text-white',
    starFilled: 'fill-primary text-primary',
    starEmpty: 'text-white/15',
  },
  minimal: {
...
export function ReviewCarousel({
  ...
  return (
    <section className={cn('px-6 py-20 md:px-12 md:py-24', styles.section, className)}>
      <div className="mx-auto w-full max-w-6xl">
        <div className="mb-10 text-center">
          <h2 className={cn('font-headline text-4xl font-extrabold uppercase tracking-tight md:text-5xl', styles.title)}>
            {title}
          </h2>
          {description ? (
            <p className={cn('mx-auto mt-5 max-w-3xl text-base leading-7 md:text-lg', styles.description)}>
              {description}
            </p>
          ) : null}
        </div>
        ...
        <div className="flex items-center justify-center gap-3 sm:gap-6">
          <button
            ...
            className={cn('flex h-11 w-11 shrink-0 items-center justify-center rounded-none transition disabled:cursor-not-allowed disabled:opacity-35', styles.button)}
          >
          ...
          >
            ...
            <div className="mt-8">
              <p className={cn('font-headline text-base font-extrabold uppercase tracking-tight md:text-lg', styles.author)}>{active.author}</p>
              <p className={cn('mt-2 text-[10px] font-bold uppercase tracking-[0.3em]', styles.meta)}>
