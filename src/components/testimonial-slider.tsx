      ...
      <div className="relative z-10 max-w-5xl mx-auto">
            ...
            {testimonials.map((t, idx) => (
              <CarouselItem key={idx}>
                <div className="relative p-12 md:p-24 bg-white/5 backdrop-blur-md border border-white/20 shadow-2xl text-center group transition-all duration-700">
                  ...
                  <blockquote className="relative z-20 font-serif italic text-3xl md:text-5xl text-[#111111] leading-[1.15] mb-12 max-w-4xl mx-auto tracking-tight">
                    "{t.quote}"
                  </blockquote>

                  <div className="flex flex-col items-center">
                    <div className="relative w-20 h-20 rounded-full overflow-hidden border-2 border-primary/20 p-1 mb-6 bg-white/10 backdrop-blur-sm">
                      <Image 
                        src={t.avatar} 
                        alt={t.author} 
                        fill
                    ...
                    <div className="space-y-1">
                      <p className="font-headline font-extrabold text-sm tracking-tight uppercase text-[#111111]">{t.author}</p>
                      <p className="text-[10px] font-bold text-gray-400 tracking-[0.3em] uppercase">{t.suburb}</p>
                    </div>
                  </div>
        ...
        <div className="flex justify-center gap-3 mt-12">
          {testimonials.map((_, i) => (
            <div key={i} className={`h-1.5 w-1.5 rounded-full transition-all duration-500 ${i === 0 ? 'bg-primary w-8' : 'bg-gray-200'}`} />
          ))}
        </div>
      </div>
    </section>
